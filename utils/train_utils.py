...
def create_model_and_loss_module(config, logger, accelerator,
    ...
    if config.experiment.get("init_weight", ""):
        ...
        if config.model.vq_model.finetune_decoder and model_type in ["titok"]:
    ...
    if accelerator.is_main_process:
        if model_type in ["titok"]:
            ...
            input_size = (1, config.model.vq_model.num_latent_tokens, config.model.vq_model.augment_ratio)
            input_data = [
...
def create_dataloader(config, logger, accelerator):
    ...
    if dataset_config.dataset_type == "pretokenized_webdataset":
    ...
    # SimpleImageDataset
    elif dataset_config.dataset_type == "simple_image_dataset":
    ...
    elif dataset_config.dataset_type == "pretokenized_dataset_json":
...
def create_evaluator(config, logger, accelerator):
    ...
    if config.model.vq_model.get("quantize_mode", "vq") in ["vq", "optvq"]:
...
def train_one_epoch(config, logger, accelerator,
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate([model, loss_module]):
            if model_type == "titok":
                reconstructed_images, extra_results_dict = model(images, use_semantic=False)
            ...
            indices = extra_results_dict["min_encoding_indices"].flatten()
            unique_ratio = len(torch.unique(indices)) / len(indices)
            autoencoder_logs = {"train/index_unique_ratio": unique_ratio}
        ...
        if accelerator.sync_gradients:
            ...
            if (global_step + 1) % config.experiment.log_every == 0:
                ...
                logger.info(
                    f"{i}/{train_dataloader.num_batches} "
                    ...
                    f"Quant Loss: {autoencoder_logs['train/quantizer_loss']:0.4f} "
                    f"Unique Ratio: {autoencoder_logs['train/index_unique_ratio']:0.4f} "
                )
                logs = {
                    "lr": lr,
                    "lr/generator": lr,
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if config.dataset.params.get("pretokenization", ""):
            ...
            if input_tokens.dim() == 2:
                B, Total_L = input_tokens.shape
                # 获取你的 augment_ratio (例如 4)
                augment_ratio = config.model.vq_model.get("augment_ratio", 1)
                
                # 只有 Multi-Token 模式下才需要 reshape
                if augment_ratio > 1:
                    # 安全检查：确保长度合法
                    if Total_L % augment_ratio != 0:
                         raise ValueError(f"Input token length {Total_L} is not divisible by augment_ratio {augment_ratio}")
                    
                    L = Total_L // augment_ratio
                    # 强制 Reshape，确保后续 Masking 是以 Group (Patch) 为单位
                    input_tokens = input_tokens.view(B, L, augment_ratio)
        else:
            # tokenize on the fly
            if "image" in batch:
                images = batch["image"].to(
                ...
                with torch.no_grad():
                    tokenizer.eval()
                    indices = tokenizer.encode(images)[1]["min_encoding_indices"]  # [B, 1, L*A]
                    B = images.shape[0]
                    L = config.model.vq_model.num_latent_tokens
                    A = config.model.vq_model.augment_ratio
                    input_tokens = indices.view(B, 1, L, A).squeeze(1)  # [B, L, A]
            else:
        ...
        with accelerator.accumulate([model]):
            ...
            if model_type == "maskgit":
                ...
                conditions,
                cond_drop_prob=config.model.generator.class_label_dropout,
                targets=input_tokens  # <--- 新增：传入干净的原始 token 作为 AR Head 的参考答案
                )
...
def reconstruct_images(model, original_images, fnames, accelerator, 
    ...
    with torch.amp.autocast("cuda", dtype=dtype, enabled=accelerator.mixed_precision != "no"):
...
def generate_images(model, tokenizer, accelerator, 
    ...
    temp = config.model.generator.get("randomize_temperature", 2.0)
    ...
    if config.training.enable_wandb:
        ...
        accelerator.get_tracker("tensorboard").log_images(
            {"Train Generated": images_for_logging}, step=global_step, dataformats="CHW"
        )
    # Log locally.
