...
def get_titok_tokenizer(config):
    if "huggingface" in config.experiment.tokenizer_checkpoint:
        hf_url = config.experiment.tokenizer_checkpoint.split(":")[-1]
        tokenizer = TiTok.from_pretrained(hf_url)
    else:
...
def sample_fn(generator,
    ...
    # 直接传入 [B, L, A]，由 TiTok.decode_tokens 兼容多路ID并合成向量
