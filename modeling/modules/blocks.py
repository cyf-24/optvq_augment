...
class TiTokEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.token_size = config.model.vq_model.token_size       # not change for later segmentation 
        ...
        # token_size = 32 , num_latent_tokens = 64 , for 
    def forward(self, pixel_values, latent_tokens):
        ...
        # # now with size B * 32 * 64 * 1
        # # now want size B * 8 * 1 * 256
        # latent_tokens = latent_tokens.reshape(batch_size, self.token_size // self.augment_ratio, 1, self.num_latent_tokens * self.augment_ratio)
        # # temprarily augment_ratio = 4
...
class TiTokDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.num_latent_tokens = config.model.vq_model.num_latent_tokens
        # self.num_latent_tokens = config.model.vq_model.num_latent_tokens * config.model.vq_model.augment_ratio
        self.token_size = config.model.vq_model.token_size
        # self.token_size = config.model.vq_model.token_size // config.model.vq_model.augment_ratio
        self.is_legacy = config.model.vq_model.get("is_legacy", True)
        self.width = {
                "small": 512,
                "base": 768,
