...
from .discriminator_dino import DinoDiscriminator
from .diff_aug import DiffAug
...
class ReconstructionLoss_Stage2(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        loss_config = config.losses
        self.discriminator_type = loss_config.discriminator_type
        # initiate the discriminator
        if self.discriminator_type == "patchgan":
            self.discriminator = NLayerDiscriminator()
        elif self.discriminator_type == "dinodisc":
            aug_prob = 1.0
    ...
    def _forward_generator(self,
        ...
        if discriminator_factor > 0.0 and self.discriminator_weight > 0.0:
            ...
            if self.discriminator_type == "patchgan":
                ...
                logits_fake = self.discriminator(
                    self.daug.aug(reconstructions.contiguous()),
                    fade_blur_schedule
                )
            else:
                raise ValueError(f"Unsupported discriminator type {self.discriminator_type}")
        ...
        loss_dict = dict(
            ...
            semantic_loss=extra_result_dict["sem_loss"].detach(),
            d_weight=d_weight,
    ...
    def _forward_discriminator(self,
        ...
        if self.discriminator_type == "patchgan":
            ...
            logits_real = self.discriminator(
                self.daug.aug(real_images),
                fade_blur_schedule
            )
            logits_fake = self.discriminator(
                self.daug.aug(reconstructions.detach()),
                fade_blur_schedule
            )
        else:
            raise ValueError(f"Unsupported discriminator type {self.discriminator_type}")
...
class MLMLoss(torch.nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor,
        ...
        支持两种输入格式：
        - 3D: [B, N, C]，单路分类（旧逻辑）
        - 4D: [B, L, C, A]，多路分类（augment_ratio=A）。此时将 A 维与 L 合并，
          并将 targets/weights 同步展平为 [B, L*A]。
        """
        if inputs.dim() == 4:
            # [B, L, V, A] -> [B, L, A, V] -> [B, L*A, V] -> [B, V, L*A]
            B, L, V, A = inputs.shape
            # inputs = inputs.permute(0, 1, 3, 2).contiguous().view(B, L * A, V).permute(0, 2, 1).contiguous()
            inputs = inputs.permute(0, 2, 1, 3).contiguous().view(B, V, L * A)
            # targets: [B, L, A] -> [B, L*A]
            if targets.dim() == 3:
                targets = targets.contiguous().view(B, L * A)
            elif targets.dim() == 2 and targets.shape[1] == L:
                # 兼容性兜底：若不小心传了 [B, L]，复制到 A 路
                targets = targets.unsqueeze(-1).expand(-1, -1, A).contiguous().view(B, L * A)
            else:
                raise ValueError(f"Unexpected targets shape {targets.shape} for inputs shape [B,{L},{V},{A}]")
            # weights: [B, L] or [B, L, A] -> [B, L*A]
            if weights is not None:
                if weights.dim() == 2:
                    weights = weights.unsqueeze(-1).expand(-1, -1, A).contiguous().view(B, L * A)
                elif weights.dim() == 3:
                    weights = weights.contiguous().view(B, L * A)
                else:
                    raise ValueError(f"Unexpected weights shape {weights.shape} for multi-head inputs")
        elif inputs.dim() == 3:
            # [B, N, C] -> [B, C, N]
            ...
            raise ValueError(f"Unexpected inputs shape {inputs.shape}. Expect 3D or 4D.")
        ...
        # 未被mask位置按较小权重计算
        loss_weights = (1.0 - weights) * self.loss_weight_unmasked_token + weights
        loss = (loss * loss_weights).sum() / (loss_weights.sum() + 1e-8)
        # 只在被mask位置统计正确率
        preds = torch.argmax(inputs, dim=1)  # [B, N]
        correct_tokens = ((preds == targets) * weights).sum(dim=1) / (weights.sum(1) + 1e-8)
