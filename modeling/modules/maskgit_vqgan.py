...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self, num_embeddings, embedding_dim, commitment_cost, augment_ratio=1):
        # 对应改titok.py中的Pixel_Quantizer的初始化
        r"""
        Args:
        ...
        assert embedding_dim % augment_ratio == 0, f"embedding_dim {embedding_dim} must be divisible by augment_ratio {augment_ratio}"
        self.head_dim = embedding_dim // augment_ratio
        self.embedding = nn.Embedding(num_embeddings, self.head_dim)
        # 共享码本
    ...
    def forward(self, hidden_states, return_loss=False):
        ...
        hidden_states_reshaped = hidden_states.view(B, H, W, self.augment_ratio, self.head_dim)
        hidden_states_flattened = hidden_states_reshaped.reshape(-1, self.head_dim)
        # distances = self.compute_distances(hidden_states)
        distances = self.compute_distances(hidden_states_flattened)
        ...
        z_q_flattened = torch.matmul(min_encodings, self.embedding.weight)
        # 还没view,先叫flattened
        z_q = z_q_flattened.view(B, H, W, self.augment_ratio, self.head_dim)
        z_q = z_q.reshape(B, H, W, C)
        # 或者直接z_q = z_q_flattened.view(B, H, W, C)

        # reshape to (batch, num_tokens)
        min_encoding_indices = min_encoding_indices.reshape(hidden_states.shape[0], -1)
        # 这里的对应ID变多了，在inference中transformer需要预测的ID多了
    ...
    def get_codebook_entry(self, indices):
        ...
        if len(indices.shape) == 2:
            ...
            batch, num_tokens_total = indices.shape
            z_q = self.embedding(indices)
            # total_tokens = H * W * Ratio
            # spatial_tokens = H * W = total / Ratio
            spatial_tokens = num_tokens_total // self.augment_ratio
            side_len = int(math.sqrt(spatial_tokens))
            # 变回图像格式 (B, C, H, W)
            # z_q = z_q.reshape(batch, int(math.sqrt(num_tokens)), int(math.sqrt(num_tokens)), -1).permute(0, 3, 1, 2)
            z_q = z_q.reshape(batch, side_len, side_len, self.embedding_dim).permute(0, 3, 1, 2)
    ...
    def get_soft_code(self, hidden_states, temp=1.0, stochastic=False):
        ...
        # 处理核心：拆分维度，合并到N里面，后面直接复用
        hidden_states = hidden_states.view(B, H, W, self.augment_ratio, self.head_dim)
        hidden_states_flattened = hidden_states.reshape(-1, self.head_dim)
        ...
        batch, num_tokens_total = code.shape
        soft_code = soft_code.reshape(batch, num_tokens_total, -1)  # (batch, height * width, num_embeddings)
        #这里保持序列长度增长，因为是对应的索引返回，后面使用要再拼起来
